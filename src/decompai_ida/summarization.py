# type: ignore
# Copied from: https://github.com/langchain-ai/langmem/blob/a463b1f86520230dcaff7822f8a73a3bacf350dd/src/langmem/short_term/summarization.py
# TODO: Currently langmem declares python 3.10 support but uses python 3.11 features,
#       that's why this file was copied instead of using langmem.
import warnings
...
def _adjust_messages_before_summarization(
    ...
    if (
        ...
        if not adjusted_messages_to_summarize:
            ...
            adjusted_messages_to_summarize = (
                preprocessed_messages.messages_to_summarize
            )
    else:
        adjusted_messages_to_summarize = (
...
def _prepare_summarization_result(
    *,
    ...
    total_summarized_messages = (
        preprocessed_messages.total_summarized_messages
        + len(preprocessed_messages.messages_to_summarize)
    )
    if running_summary:
        ...
        include_system_message = (
            preprocessed_messages.existing_system_message
            and not (
                existing_summary
...
def summarize_messages(
    ...
    *,
    ...
    summarized_message_ids = (
        set(running_summary.summarized_message_ids)
        if running_summary
        else set()
    )
    if preprocessed_messages.messages_to_summarize:
        ...
        summarized_message_ids = summarized_message_ids | set(
            message.id
            for message in preprocessed_messages.messages_to_summarize
        )
